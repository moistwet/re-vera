...
describe('parseSSE', () => {
  ...
  it('delivers a message that carries no id: line at all', async () => {
    // The backend's stream-deadline `error: timeout` is generated by the relay,
    // was never written to the job's replay list, and so borrows no sequence
    // number. Per spec a missing `id:` leaves the last event id untouched, and
    // Re-Vera reports the id only for the message that actually carried one —
    // so this message must arrive with `id` simply absent, not inherited from
    // the event before it.
    const messages = await collect(
      streamOf(
        'id: 1\nevent: claims_found\ndata: {"type":"claims_found","count":1,"claim_ids":["c1"]}\n\n' +
          'event: error\ndata: {"type":"error","code":"timeout","message":"This check stopped responding before it finished. Please try checking the article again."}\n\n',
      ),
    )

    expect(messages).toHaveLength(2)
    expect(messages[1].event).toBe('error')
    expect(messages[1].id).toBeUndefined()
    expect('id' in messages[1]).toBe(false)
    expect(JSON.parse(messages[1].data).code).toBe('timeout')
  })
